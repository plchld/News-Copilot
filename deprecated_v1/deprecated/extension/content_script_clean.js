...
function renderViewpoints(viewpointsData, citationsData, panel) {
    ...
    // Handle multiple formats: new flexible schema, old array format, and legacy string
    let parsedViewpoints = null;
    let isFlexibleSchema = false;
    
    try {
        if (typeof viewpointsData === 'string') {
            parsedViewpoints = JSON.parse(viewpointsData);
        } else if (typeof viewpointsData === 'object') {
            parsedViewpoints = viewpointsData;
        }
        
        // Check if it's the new flexible schema
        if (parsedViewpoints && typeof parsedViewpoints === 'object' && 
            (parsedViewpoints.alternative_perspectives || parsedViewpoints.topic_analysis)) {
            isFlexibleSchema = true;
        }
    } catch (e) {
        parsedViewpoints = null;
    }
    
    if (isFlexibleSchema && parsedViewpoints) {
        // New flexible schema format
        ...
        contextIntro.textContent = 'Εναλλακτικές οπτικές και ανάλυση για το θέμα του άρθρου:';
        section.appendChild(contextIntro);
        
        const formattedContent = formatFlexibleViewpoints(parsedViewpoints);
        section.insertAdjacentHTML('beforeend', formattedContent);
    } else if (parsedViewpoints && Array.isArray(parsedViewpoints) && parsedViewpoints.length > 0) {
...
function convertMarkdownToHtml(markdown) {
    ...
    let html = markdown
        // Headers (## Heading)
        .replace(/^## (.+)$/gm, '<h4 style="margin: 20px 0 10px 0; color: #1e293b; font-weight: 600; font-size: 16px; border-bottom: 2px solid #e2e8f0; padding-bottom: 4px;">$1</h4>')
        // Bold text (**text**)
    ...
    if (!html.includes('<p>') && !html.includes('<h4>')) {
        html = `<p style="margin: 12px 0; line-height: 1.6;">${html}</p>`;
    } else if (html.includes('<h4>') && !html.startsWith('<h4>')) {
        // Add opening paragraph tag if content starts before first heading
        html = `<p style="margin: 12px 0; line-height: 1.6;">${html}</p>`;
    }
...
// --- New Flexible Schema Formatting ---
function formatFlexibleViewpoints(data) {
    try {
        let html = '';
        
        // Add topic analysis if present
        if (data.topic_analysis) {
            html += `
                <div style="margin-bottom: 20px; padding: 16px; background: #f1f5f9; border-radius: 8px; border-left: 4px solid #3b82f6;">
                    <div style="font-size: 12px; color: #475569; font-weight: 600; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px;">
                        📋 Ανάλυση Θέματος
                    </div>
        ...
        // Add alternative perspectives - the main content with markdown conversion
        if (data.alternative_perspectives) {
            html += `
                <div style="margin-bottom: 20px;">
                    <div style="font-size: 12px; color: #475569; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                        🌐 Εναλλακτικές Οπτικές
                    </div>
                    <div style="font-size: 14px; color: #334155; line-height: 1.7;">
                        ${convertMarkdownToHtml(linkifyXPosts(data.alternative_perspectives))}
                    </div>
                </div>
            `;
        }
        
        // Add key sources if present
        if (data.key_sources && Array.isArray(data.key_sources) && data.key_sources.length > 0) {
            html += `
                <div style="margin-top: 16px; padding: 16px; background: #fefce8; border-radius: 8px; border-left: 4px solid #eab308;">
                    <div style="font-size: 12px; color: #713f12; font-weight: 600; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 0.5px;">
                        📚 Κύριες Πηγές
                    </div>
                    ${data.key_sources.map(source => `
                        <div style="margin-bottom: 12px; padding-bottom: 12px; border-bottom: 1px solid #fde047;">
                            <div style="font-weight: 600; color: #92400e; margin-bottom: 4px;">${source.source}</div>
                            ${source.perspective_summary ? `<div style="font-size: 13px; color: #a16207; line-height: 1.5;">${source.perspective_summary}</div>` : ''}
                        </div>
                    `).join('')}
                </div>
            `;
        ...
        return html || '<div style="padding: 20px; text-align: center; color: #6b7280;">Δεν υπάρχουν δεδομένα για εμφάνιση</div>';
        
    } catch (error) {
        console.error('Error formatting flexible viewpoints:', error);
        return '<div style="padding: 20px; text-align: center; color: #dc2626;">Σφάλμα μορφοποίησης περιεχομένου</div>';
    }
}

// --- Legacy Structured Viewpoints Formatting ---
