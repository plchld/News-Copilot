...
class OptimizedCoordinatorConfig:
    ...
    enable_context_caching: bool = False  # Disabled by default
...
class CoreAnalysisCoordinator:
    ...
    async def execute_core_analysis(
        ...
        try:
            ...
            self.logger.info(
                f"[CORE_RESULTS] {session_id} - Jargon | "
                f"Success: {jargon_result.success} | "
                f"Data keys: {list(jargon_result.data.keys()) if jargon_result.data else 'None'} | "
                f"Items: {len(jargon_result.data.get('terms', [])) if jargon_result.data else 0}"
            )
            
            self.logger.info(
                f"[CORE_RESULTS] {session_id} - Viewpoints | "
                f"Success: {viewpoints_result.success} | "
                f"Data keys: {list(viewpoints_result.data.keys()) if viewpoints_result.data else 'None'} | "
                f"Items: {len(viewpoints_result.data.get('viewpoints', [])) if viewpoints_result.data else 0}"
            )
...
class OnDemandCoordinator:
    ...
    async def execute_on_demand(
        ...
        # Get enhanced context from cache or create directly if caching is disabled
        if self.config.enable_context_caching:
            ...
            # Caching disabled - create context directly from user_context
            if not user_context or not user_context.get('article_text') or not user_context.get('article_url'):
                error_msg = f"When caching is disabled, article_text and article_url must be provided in user_context"
                self.logger.error(f"[ON_DEMAND] {session_id} - {error_msg}")
                return {
                    'success': False,
                    'error': error_msg,
                    'requires_article_data': True
                }
            
            # Create enhanced context directly
            enhanced_context = {
                'article_url': user_context['article_url'],
                'article_text': user_context['article_text'],
                'session_id': session_id,
                'request_type': RequestType.ON_DEMAND.value,
                'analysis_type': analysis_type,
                'has_core_results': False,
                'cache_hit': False,
                'user_tier': user_context.get('user_tier', 'free'),
                'user_id': user_context.get('user_id'),
                **user_context
            ...
            self.logger.info(f"[ON_DEMAND] {session_id} - Created context directly (caching disabled)")
