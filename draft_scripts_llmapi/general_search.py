...
class SearchClient:
    ...
    def _normalize_country_code(self, country: str) -> str:
        ...
        # If not found, default to United States
        print(f"⚠️  Unknown country '{country}', defaulting to United States (US)")
        return 'US'
    
    async def search_with_claude(self, system_prompt: str, user_prompt: str, country: str = 'US') -> dict:
        ...
        location_info = CITY_MAPPING.get(country_code, CITY_MAPPING['US'])
    ...
    def search_with_grok(self, system_prompt: str, user_prompt: str, sources: list = None, country: str = 'US') -> str:
        ...
        if response.status_code == 200:
            ...
            # Add citations if available (in English)
...
async def perform_search(client_type: str = "claude", prompt_file: str = "search_prompt.md", 
    ...
    try:
        ...
        # Add date and country context to user prompt (in English)
        enhanced_user_prompt = f"{user_prompt}\n\nDate: {current_day} ({today})\nCountry focus: {country.upper()}"
        
        # For Claude, add explicit web search instruction (in English)
        if client_type == "claude":
            enhanced_user_prompt += "\n\nUSE the web search tool NOW to find the most recent information. Do not respond without searching first."
        
        # Only add Greek language instruction at the very end
        enhanced_user_prompt += "\n\n translate the response in Greek language."
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='US',
        help='Country to focus search on (ISO alpha-2 code or name, default: US)'
    )
